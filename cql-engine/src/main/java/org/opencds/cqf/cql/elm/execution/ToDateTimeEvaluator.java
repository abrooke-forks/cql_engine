...
public class ToDateTimeEvaluator extends org.cqframework.cql.elm.execution.ToDateTime {
    ...
    public static Object toDateTime(Object operand) {
        if (operand == null) {
            return null;
        }

        if (operand instanceof String) {
            return new DateTime((String) operand);
        }

        throw new IllegalArgumentException(String.format("Cannot ToDateTime arguments of type '%s'.", operand.getClass().getName()));
    }

    @Override
    public Object evaluate(Context context) {
        Object operand = getOperand().evaluate(context);
        return context.logTrace(this.getClass(), toDateTime(operand), operand);
    }
}
